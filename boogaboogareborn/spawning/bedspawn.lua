...
--[[
the game connects to `...BedButton.Activated:Connect(function(...) end)`
instead of using replicatesignal or firesignal which have been proven to hardly work, we choose the next best thing
since filtergc already finds the Activated callback function, we can use getconnections to find every `...Activated:Connect()` rbxscriptconnection (which also retains the function(s) for each connection)
...
(i believe that this one is better because there's no filtering through gc, and no constant comparison. it's the cleanest and the most straightforward.)
]]
